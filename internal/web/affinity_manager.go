...
func loadAffinityConfig() affinityConfig {
	...
	return affinityConfig{
		...
		AccountConcurrency: affinityEnvInt("M365_ACCOUNT_DEFAULT_CONCURRENCY", 8),
	}
}
...
func openAffinityManager(config affinityConfig) *affinityManager {
	...
	if config.Mode == affinityOff || config.RedisURL == "" {
		m.degraded = config.Mode != affinityOff && config.RedisURL == ""
		if m.degraded {
			m.lastError = "M365_REDIS_URL is empty"
		}
		return m
	}
...
func (m *affinityManager) markStoreError(err error) affinityStore {
	if err != nil {
		...
		m.lastError = err.Error()
		m.mu.Unlock()
		log.Printf("[affinity] redis degraded: %v", err)
	}
	return m.fallback
}
...
func (m *affinityManager) begin(ctx context.Context, tenant string, body *oaiReq, r *http.Request, accounts []auth.AccountToken, available func(string) bool) (*affinityRequest, error) {
	...
	if state.hasBinding && sharedAvailable(state.binding.AccountID) && (state.prefixCount > 0 || state.key.BindingID != "") {
		...
		if accountID, ok, err := state.store.GetAccount(ctx, state.key.TenantHash, state.key.Hash); err == nil && ok && sharedAvailable(accountID) {
			...
			m.mu.Lock()
			load := make(map[string]int, len(m.inflight))
			for id, count := range m.inflight {
				load[id] = count
			}
			m.mu.Unlock()
			if selected, ok := selectRendezvousAccount(state.key.Hash, accounts, sharedAvailable, load, m.config.AccountConcurrency); ok {
	...
	if state.enforced && state.accountID != "" {
		m.mu.Lock()
		m.inflight[state.accountID]++
		m.mu.Unlock()
		state.releaseSlot = func() {
			m.mu.Lock()
			if m.inflight[state.accountID] <= 1 {
				delete(m.inflight, state.accountID)
			} else {
				m.inflight[state.accountID]--
			}
			m.mu.Unlock()
...
func (state *affinityRequest) switchAccount(accountID string) {
	...
	if state.releaseSlot != nil {
		state.releaseSlot()
		state.releaseSlot = nil
	}
	state.accountID = accountID
	state.manager.mu.Lock()
	state.manager.inflight[accountID]++
	state.manager.mu.Unlock()
	state.releaseSlot = func() {
		state.manager.mu.Lock()
		if state.manager.inflight[accountID] <= 1 {
			delete(state.manager.inflight, accountID)
		} else {
			state.manager.inflight[accountID]--
		}
		state.manager.mu.Unlock()
...
func (state *affinityRequest) complete(ctx context.Context, body *oaiReq, accountID, conversationID, sessionID string, assistant oaiMsg, promptTokens, completionTokens int64) reuseUsage {
	...
	history := append(cloneMessages(body.Messages), assistant)
