...
func selectRendezvousAccount(key string, accounts []auth.AccountToken, available func(string) bool) (auth.AccountToken, bool) {
	var bestScore uint64
	var best auth.AccountToken
	found := false
	for _, account := range accounts {
		if account.ID == "" || !available(account.ID) {
			continue
		}
		...
		if !found || raw > bestScore {
			bestScore, best, found = raw, account, true
		}
	}
	return best, found
}
...
func (s *memoryAffinityStore) wakeWaiterLocked(key string) {
...
func (s *memoryAffinityStore) removeWaiter(key string, waiter chan struct{}) {
	s.mu.Lock()
	defer s.mu.Unlock()
	waiters := s.waiters[key]
...
func (s *memoryAffinityStore) GetAccount(_ context.Context, tenantHash, affinityHash string) (string, bool, error) {
	...
	now := time.Now()
	s.cleanupLocked(now)
	key := accountStoreKey(tenantHash, affinityHash)
	v, ok := s.accounts[key]
...
func (s *memoryAffinityStore) GetResponse(_ context.Context, tenantHash, responseHash string) (string, bool, error) {
	...
	now := time.Now()
	s.cleanupLocked(now)
	key := accountStoreKey(tenantHash, responseHash)
...
func (s *memoryAffinityStore) Acquire(ctx context.Context, key string, ttl, wait time.Duration) (func(), error) {
	...
	for {
		...
		now := time.Now()
		s.cleanupLocked(now)
		if current, exists := s.locks[key]; exists && now.After(current.ExpiresAt) {
			delete(s.locks, key)
			s.wakeWaiterLocked(key)
		}
