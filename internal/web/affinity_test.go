...
func TestRendezvousSelectionIsStableAndSkipsUnavailableAccounts(t *testing.T) {
	...
	load := map[string]int{"a": 0, "b": 0, "c": 0}
	first, ok := selectRendezvousAccount("affinity", accounts, available, load, 8)
	if !ok || first.ID == "b" {
		t.Fatalf("unexpected selection: %+v ok=%v", first, ok)
	}
	for i := 0; i < 20; i++ {
		next, ok := selectRendezvousAccount("affinity", accounts, available, load, 8)
	...
	if !ok || next.ID == first.ID || next.ID == "b" {
		t.Fatalf("full/unavailable account was selected: %+v", next)
	}
}
