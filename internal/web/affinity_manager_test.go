...
func TestRedisOutageFallsBackWithoutCacheClaim(t *testing.T) {
	mr := miniredis.RunT(t)
	manager := openAffinityManager(affinityConfig{Mode: affinityEnforce, RedisURL: "redis://" + mr.Addr() + "/0", RedisPoolSize: 2, TTL: time.Hour, MaxSessions: 100, LockTTL: time.Minute, LockWait: time.Second})
	defer manager.close()
	mr.Close()
